...
static esp_err_t run_sync_sequence(void)
{
    ...
    ESP_LOGI(TAG, "Step 2/4: Upload NC program to all nodes");
    for (size_t i = 0; i < ROBOT_NODE_COUNT; i++) {
        uint8_t node_id = ROBOT_NODE_IDS[i];
        esp_err_t err = can_master_upload_nc_program(node_id,
                                                     PROGRAM_SLOT,
                                                     (const uint8_t *)NC_PROGRAM,
                                                     strlen(NC_PROGRAM),
                                                     CAN_MASTER_RESPONSE_TIMEOUT_MS);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "NC upload failed for node=%u", (unsigned)node_id);
            return err;
        }
    }
